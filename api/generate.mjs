  ...
  try {
    ...
    const titleCompletion = await client.chat.completions.create({
      ...
      messages: [
        ...
        { role: 'user', content: `Provide a concise, engaging title (max 10 words) for the following blog post. Do not include any bold markdown syntax (**):\n\n${aiText}` }
      ],
      max_tokens: 30,
      temperature: 0.5,
    });
    dynamicTitle = titleCompletion.choices[0].message.content.trim();
    // Remove any stray asterisks or quotes
  ...
  try {
    ...
    const descCompletion = await client.chat.completions.create({
      ...
      messages: [
        ...
        { role: 'user', content: `Write a single-sentence summary for a blog post. Keep it on point and omit any bold markdown (**):\n\n${aiText}` }
      ],
      max_tokens: 60,
      temperature: 0.7,
    });
    ...
    // Remove any stray asterisks or quotes
    dynamicDescription = dynamicDescription.replace(/\*/g, '');
    dynamicDescription = dynamicDescription.replace(/['"]/g, '');
